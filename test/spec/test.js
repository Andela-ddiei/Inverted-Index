...
describe('Read Book Data', () => {
    ...
    it('should check that JSON file is not empty', () => {
        expect(newIndex.createIndex('emptyBooks.json', emptyBook)).to.eql('JSON file is Empty');
        expect(newIndex.createIndex('Books.json', books)).to.not.eql('JSON file is Empty');
    });
});
...
describe('Search index', () => {
    ...
    it('should handle a varied number of search terms', () => {
        expect(newIndex.searchIndex('a alice alliance', 'books.json')).to.eql({ 'a': [0, 1, 2], 'alice': [0], 'alliance': [1, 2] });
    });
});