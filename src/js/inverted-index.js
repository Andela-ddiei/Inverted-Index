...
class InvertedIndex {
  ...
  searchIndex(query, indexToSearch) {
    ...
    searchTerms.forEach((word) => {
      ...
      if (indexToSearch) {
          searchResult[word] = this.indices[indexToSearch][word] ? this.indices[indexToSearch][word] : errorMessage;
      } else {
        Object.keys(this.indices).forEach((key) => {
            searchResult[word] = this.indices[key][word] ? this.indices[key][word] : errorMessage;
        });
      }
    });
    return searchResult;
